 ...
 * derived knowledge, plans, or beliefs. The Agent uses inference and planning (which use the KB)
 * to interact with the Environment, learning new Atoms that refine its KB and future behavior.
 ...
 * - **Advanced KB:** More efficient indexing, robust atom revision, integrated time provider.
...
public final class Cog {
    ...
    // Certainty
    private static final double CERTAINTY_DEFAULT_SENSITIVITY = 1.0; // k in confidence = count / (count + k)
    private static final double CERTAINTY_MIN_CONFIDENCE_FOR_VALID_EVIDENCE = 0.05;
    private static final double CERTAINTY_MIN_STRENGTH_FOR_RELEVANCE = 0.01;
    ...
    private static final int FORGETTING_MAX_KB_SIZE_TRIGGER = 15000; // Check if KB exceeds this size
    private static final int FORGETTING_TARGET_KB_SIZE_FACTOR = 80; // Target size as % of max after forgetting
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            cle.learnLink(Link.Type.INHERITANCE, Certainty.of(0.95, 20), 0.8, cat, mammal);
            cle.learnLink(Link.Type.INHERITANCE, Certainty.of(0.98, 50), 0.9, mammal, animal);
            cle.learnLink(Link.Type.INHERITANCE, Certainty.of(0.90, 30), 0.8, dog, mammal);
            cle.learnLink(Link.Type.INHERITANCE, Certainty.of(0.7, 10), 0.6, cat, predator);
            cle.learnLink(Link.Type.INHERITANCE, Certainty.of(0.8, 15), 0.7, predator, hasClaws);
            ...
            var agentAtA = cle.learnLink(Link.Type.HOLDS_AT, Certainty.TRUE, 1.0, cle.timePoint(t0),
                    cle.learnLink(Link.Type.EVALUATION, Certainty.TRUE, 1.0, atLocationPred, agentNode, locationA));
            // NOT HoldsAt(Fluent:HasKey(Self), T0) - Represented by low strength/confidence
            var notHasKey = cle.learnLink(Link.Type.HOLDS_AT, Certainty.FALSE, 1.0, cle.timePoint(t0),
                    cle.learnLink(Link.Type.EVALUATION, Certainty.TRUE, 1.0, hasKeyFluent, agentNode)); // Evaluation represents the state itself
            // NOT HoldsAt(Fluent:DoorOpen(Door), T0)
            var notDoorOpen = cle.learnLink(Link.Type.HOLDS_AT, Certainty.FALSE, 1.0, cle.timePoint(t0),
                    cle.learnLink(Link.Type.EVALUATION, Certainty.TRUE, 1.0, doorOpenFluent, door));
            ...
            Atom effectPickup = cle.learnLink(Link.Type.HOLDS_AT, Certainty.TRUE, 0.9, // Effect will be true
                    cle.learnLink(Link.Type.EVALUATION, Certainty.TRUE, 1.0, hasKeyFluent, agentNode));
            // Rule: Sequence(Precond, Action) => Effect [Time: duration]
            var pickupSeq = cle.learnLink(Link.Type.SEQUENCE, Certainty.TRUE, 0.9, precondPickup, pickupAction);
            cle.learnLink(Link.Type.PREDICTIVE_IMPLICATION, Certainty.of(0.95, 10), 0.9, cle.duration(100), pickupSeq, effectPickup);
            ...
            Atom effectOpen = cle.learnLink(Link.Type.HOLDS_AT, Certainty.TRUE, 0.9,
                    cle.learnLink(Link.Type.EVALUATION, Certainty.TRUE, 1.0, doorOpenFluent, door));
            // Rule: Sequence(Precond, Action) => Effect
            var openSeq = cle.learnLink(Link.Type.SEQUENCE, Certainty.TRUE, 0.9, precondOpen, openAction);
            cle.learnLink(Link.Type.PREDICTIVE_IMPLICATION, Certainty.of(0.9, 10), 0.9, cle.duration(200), openSeq, effectOpen);

            // Precondition: HoldsAt(DoorOpen(Door))
            var precondMove = effectOpen;
            // Effect: HoldsAt(AtLocation(Self, LocationB))
            Atom effectMove = cle.learnLink(Link.Type.HOLDS_AT, Certainty.TRUE, 0.9,
                    cle.learnLink(Link.Type.EVALUATION, Certainty.TRUE, 1.0, atLocationPred, agentNode, locationB));
            // Rule: Sequence(Precond, Action) => Effect
            var moveSeq = cle.learnLink(Link.Type.SEQUENCE, Certainty.TRUE, 0.9, precondMove, moveToBAction);
            cle.learnLink(Link.Type.PREDICTIVE_IMPLICATION, Certainty.of(0.98, 10), 0.9, cle.duration(300), moveSeq, effectMove);
            ...
            Atom goalState = cle.learnLink(Link.Type.HOLDS_AT, Certainty.TRUE, 1.0,
                    cle.learnLink(Link.Type.EVALUATION, Certainty.TRUE, 1.0, agentAtPred, agentNode, goalLocationNode));
            ...
            System.out.println("KB size before wait: " + sizeBefore);
            ...
            System.out.println("KB size after wait: " + sizeAfter + " (Removed: " + (sizeBefore - sizeAfter) + ")");
     ...
     * @return The resulting Atom in the KB (potentially the existing one after revision).
     */
    public Atom learn(Atom atom) {
        return mem.learnAtom(atom);
    }
    ...
    public Node getOrCreateNode(String name) {
        return mem.node(name, Certainty.UNKNOWN, IMPORTANCE_INITIAL_STI);
    }
     ...
     * @return The existing or newly created Node.
     */
    public Node getOrCreateNode(String name, Certainty certainty, double initialSTI) {
     ...
     */
    public Link learnLink(Link.Type type, Certainty certainty, double initialSTI, Atom... targets) {
     ...
     * @return The learned Link Atom.
     */
    public Link learnLink(Link.Type type, Certainty certainty, double initialSTI, Time time, Atom... targets) {
     ...
     */
    public Link learnLinkByIds(Link.Type type, Certainty certainty, double initialSTI, String... targetIds) {
     ...
     * @return The learned Link Atom, or null if target IDs are invalid.
     */
    public Link learnLinkByIds(Link.Type type, Certainty certainty, double initialSTI, Time time, String... targetIds) {
    ...
    // --- Getters for internal components (for debugging/inspection) ---
    public Memory getKnowledgeBase() {
        return mem;
    }

    public Inference getInferenceEngine() {
        return inference;
    }
    ...
    // ========================================================================
    // === Interfaces and Example Environment                             ===
    // ========================================================================
    public interface Game {
         ...
         */
        List<Atom> getAvailableActions(); // Should return Action schema Atoms (e.g., Nodes)

        /**
         * Execute the chosen action schema in the environment.
         */
        Act executeAction(Atom actionSchema);
    ...
    public static class Memory {
        ...
        public Atom learnAtom(Atom atom) {
            ...
            var result = atoms.compute(atomId, (id, existingAtom) -> {
                if (existingAtom == null) {
                    ...
                    var oldCertainty = existingAtom.certainty();
                    var revisedCertainty = oldCertainty.merge(atom.certainty());
                    existingAtom.setCertainty(revisedCertainty); // Update certainty in place
                    existingAtom.updateAccessTime(currentTime);

                    var boost = revisionBoost(oldCertainty, atom.certainty(), revisedCertainty);
            ...
            // Trigger forgetting check if KB grows too large (asynchronously)
            if (atoms.size() > FORGETTING_MAX_KB_SIZE_TRIGGER) {
         ...
         */
        public Node node(String name, Certainty certainty, double initialSTI) {
            var nodeId = Node.id(name);
        ...
        public Node node(String name) {
            return (Node) atoms.get(Node.id(name));
        }
         ...
         */
        public Link link(Link.Type type, Certainty certainty, double initialSTI, Time time, String... targetIds) {
        ...
        private Link link(Link.Type type, Certainty certainty, double initialSTI, Time time, List<String> targetIds) {
            var link = new Link(type, targetIds, certainty, time);
            link.importance(initialSTI, this.time.get());
            return (Link) learnAtom(link); // Use learnAtom for revision/indexing logic
        }
        ...
        /**
         * Gets the current number of atoms in the KB.
         */
        ...
        public synchronized void decayAndForget() {
            ...
            // 2. Forget low-importance atoms if KB is large
            var targetSize = (FORGETTING_MAX_KB_SIZE_TRIGGER * FORGETTING_TARGET_KB_SIZE_FACTOR) / 100;
            if (initialSize > FORGETTING_MAX_KB_SIZE_TRIGGER || initialSize > targetSize * 1.1) { // Check size triggers
            ...
            if (removedCount > 0 || decayCount > 0) {
                System.out.printf("DEBUG: Maintenance Cycle: Decayed %d atoms. Removed %d atoms (below importance < %.3f). KB size %d -> %d.%n",
                        decayCount, removedCount, IMPORTANCE_MIN_FORGET_THRESHOLD, initialSize, atoms.size());
            }
        }

        private double revisionBoost(Certainty oldC, Certainty newC, Certainty revisedC) {
            var strengthChange = Math.abs(revisedC.strength - oldC.strength);
            var confidenceChange = Math.abs(revisedC.confidence() - oldC.confidence());
            // Boost more for significant changes or high-confidence confirmations
            var boost = (strengthChange * 0.6 + confidenceChange * 0.4) * (1.0 + newC.confidence());
    ...
    public static class Inference {
        ...
        public Optional<Link> deduce(Link linkAB, Link linkBC) {
            if (!isValidBinaryLink(linkAB) || !isValidBinaryLink(linkBC) || linkAB.type != linkBC.type)
                return empty();
            ...
            var cAB = linkAB.certainty();
            var cBC = linkBC.certainty();
            var cB = nodeBOpt.get().certainty();
            var cC = nodeCOpt.get().certainty(); // Base rates //TODO utilize 'cC' somehow
            ...
            var cBConf = cB.confidence();
            var sAC = unitize(
                (cABstrength * cBCstrength * (1 - cBConf)) // Product term
                + (cABstrength * cBCstrength + (1 - cABstrength) * cBCstrength * cAB.confidence()) * cBConf // Towards implication
            );

            var discount = (linkAB.type == Link.Type.PREDICTIVE_IMPLICATION) ? INFERENCE_TEMPORAL_DISCOUNT : INFERENCE_CONFIDENCE_DISCOUNT;
            var nAC = discount * Math.min(cAB.count, cBC.count) * cBConf; // Confidence depends on middle term's confidence
            var cAC = Certainty.of(sAC, nAC);

            if (cAC.confidence() < CERTAINTY_MIN_CONFIDENCE_FOR_VALID_EVIDENCE) return empty();

            var timeAC = (linkAB.type == Link.Type.PREDICTIVE_IMPLICATION) ?
                    Time.compose(linkAB.time().get(), linkBC.time().get()) : null;
            ...
            return Optional.of((Link)mem.learnAtom(inferredLink));
        }
        ...
        public Optional<Link> invert(Link linkAB) {
            ...
            if (cB.strength < 1e-9) { // Avoid division by zero; P(B) approx 0
                ...
                nBA = discount * cAB.count * cA.confidence() * cB.confidence();
            }
            var cBA = Certainty.of(sBA, nBA);

            if (cBA.confidence() < CERTAINTY_MIN_CONFIDENCE_FOR_VALID_EVIDENCE) return empty();

            var timeBA = (linkAB.type == Link.Type.PREDICTIVE_IMPLICATION && linkAB.time() != null) ?
                    linkAB.time().get().inverse() : null;
        ...
        public Optional<Atom> modusPonens(Atom premiseA, Link implicationAB) {
            ...
            var bId = implicationAB.targets.get(1);
            var conclusionBOpt = atom(bId);
            if (conclusionBOpt.isEmpty()) return empty(); // Conclusion atom must exist to be updated

            var cA = premiseA.certainty();
            var cAB = implicationAB.certainty();
            ...
            var derived_sB = cA.strength * cAB.strength;
            ...
            var derived_nB = discount * Math.min(cA.count, cAB.count);

            if (derived_nB < 1e-6) return empty(); // Negligible evidence

            var evidenceCertainty = Certainty.of(derived_sB, derived_nB);
            ...
            var revisedB = mem.learnAtom(evidenceForB);
        ...
        private PriorityQueue<PotentialInference> gatherPotentialInferences() {
            var queue = new PriorityQueue<PotentialInference>(
                    Comparator.<PotentialInference, Double>comparing(inf -> inf.priority).reversed()
            );
            final long currentTime = mem.time.get();

            Predicate<Atom> isConfidentEnough = a -> a.certainty().confidence() > CERTAINTY_MIN_CONFIDENCE_FOR_VALID_EVIDENCE;
            ...
            Stream.of(Link.Type.INHERITANCE, Link.Type.PREDICTIVE_IMPLICATION)
                    ...
                    .forEach(linkAB -> {
                        ...
                        mem.linksWithTarget(bId) // Find links *starting* with B: B->C
                                .filter(linkBC -> {
                                    return linkBC.type == linkAB.type && isValidBinaryLink(linkBC) && linkBC.targets.get(0).equals(bId);
                                })
            ...
            Stream.of(Link.Type.INHERITANCE, Link.Type.PREDICTIVE_IMPLICATION)
                    ...
                    .filter(this::isValidBinaryLink).filter(isConfidentEnough)
                    .forEach(linkAB -> {
                        atom(linkAB.targets.get(0)).ifPresent(premiseA -> {
                ...
                if (!executedSignatures.contains(signature)) {
                    ...
                    if (premisesValid) {
                        switch (potential.ruleType) {
                            case DEDUCTION:
                                resultOpt = deduce((Link) potential.premises[0], (Link) potential.premises[1]);
                                break;
                            case INVERSION:
                                resultOpt = invert((Link) potential.premises[0]);
                                break;
                            case MODUS_PONENS:
                                resultOpt = modusPonens(potential.premises[0], (Link) potential.premises[1]);
                                break;
        ...
        public List<Answer> backwardChain(Atom queryPattern, int maxDepth) {
            return backwardChainRecursive(queryPattern, maxDepth, Bindings.EMPTY_BINDINGS, new HashSet<>());
        }
        ...
        private List<Answer> backwardChainRecursive(Atom queryPattern, int depth, Bindings bindings, Set<String> visited) {
            ...
            results.addAll(tryModusPonensBackward(currentQuery, nextDepth, bindings, visited));

            // Try Deduction Backward: Query A->C, find A->B and B->C
            if (currentQuery instanceof Link queryLink && isValidBinaryLink(queryLink))
                results.addAll(tryDeductionBackward(queryLink, nextDepth, bindings, visited));

            // Try Inversion Backward: Query B->A, find A->B
            if (currentQuery instanceof Link queryLink && isValidBinaryLink(queryLink))
                results.addAll(tryInversionBackward(queryLink, nextDepth, bindings, visited));


            // Try ForAll Instantiation Backward: Query P(a), find ForAll(?X, P(?X)) rule
            results.addAll(tryForAllBackward(currentQuery, nextDepth, bindings, visited));
        ...
        private List<Answer> tryModusPonensBackward(Atom queryB, int depth, Bindings bindings, Set<String> visited) {
            List<Answer> results = new ArrayList<>();
        ...
        private List<Answer> tryDeductionBackward(Link queryAC, int depth, Bindings bindings, Set<String> visited) {
            List<Answer> results = new ArrayList<>();
            ...
            mem.links(queryAC.type)
                    ...
                    .forEach(ruleAB -> {
                        ...
                        unify.unifyAtomId(aIdQuery, aIdRule, bindings).ifPresent(bindingsA -> {
                            ...
                            var subgoalBCPattern = new Link(queryAC.type, List.of(bIdRule, cIdQuery), Certainty.UNKNOWN, null);
        ...
        private List<Answer> tryInversionBackward(Link queryBA, int depth, Bindings bindings, Set<String> visited) {
            List<Answer> results = new ArrayList<>();
        ...
        private List<Answer> tryForAllBackward(Atom queryInstance, int depth, Bindings bindings, Set<String> visited) {
            List<Answer> results = new ArrayList<>();
            mem.links(Link.Type.FOR_ALL).forEach(forAllLink -> {
                if (forAllLink.targets.size() >= 2) { // Should be FOR_ALL(Var1, Var2..., Body)
                    var varIds = forAllLink.targets.subList(0, forAllLink.targets.size() - 1);
                    var bodyId = forAllLink.targets.get(forAllLink.targets.size() - 1);

                    atom(bodyId).ifPresent(bodyPattern -> {
                        // Try to unify the query instance with the body pattern
                        unify.unify(bodyPattern, queryInstance, bindings).ifPresent(matchBindings -> {
                            ...
                            for (var resForAll : forAllResults) {
                                ...
                                var supportedInstance = finalInstance.withCertainty(resForAll.inferredAtom.certainty());
        ...
        private Optional<List<Atom>> planRecursive(Atom currentGoalPattern, LinkedList<Atom> currentPlan, int planDepthRemaining, int searchDepth, Set<String> visitedGoalPatterns) {
            ...
            var bestResult = goalResults.stream()
                    .max(Comparator.comparingDouble(r -> r.inferredAtom.certainty().strength * r.inferredAtom.certainty().confidence()));

            if (bestResult.isPresent() && bestResult.get().inferredAtom.certainty().strength > 0.7 && bestResult.get().inferredAtom.certainty().confidence() > 0.5) {
            ...
            for (var step : potentialSteps) {
                ...
                Set<String> branchVisited = new HashSet<>(visitedGoalPatterns);
        ...
        /**
         * Finds actions that might achieve the goalPattern based on KB rules.
        ...
        private List<PotentialPlanStep> findActionsAchieving(Atom goalPattern, int searchDepth) {
            ...
            mem.links(Link.Type.PREDICTIVE_IMPLICATION)
                    ...
                    .forEach(rule -> {
                        ...
                        unify.unify(consequenceAtom, goalPattern, Bindings.EMPTY_BINDINGS).ifPresent(bindings -> {
                            ...
                            atom(premiseId).ifPresent(premiseAtom -> {
                                ...
                                if (premiseAtom instanceof Link premiseLink && premiseLink.type == Link.Type.SEQUENCE) {
                                    ...
                                    actionOpt.ifPresent(actionAtom -> {
                                        ...
                                        var preconditions = premiseLink.targets.stream()
                                                ...
                                                .toList();
                                        var confidence = rule.certainty().confidence() * premiseLink.certainty().confidence();
                                ...
                                // Case 2: Premise is directly an Action (implies no preconditions in this rule)
                                    ...
                                    steps.add(new PotentialPlanStep(premiseAtom, Collections.emptyList(), rule.certainty().confidence()));
                                }
        ...
        private Optional<LinkedList<Atom>> satisfyPreconditions(List<Atom> preconditions, LinkedList<Atom> currentPlan, int planDepthRemaining, int searchDepth, Set<String> visitedGoalPatterns) {
            ...
            var nextPrecondition = preconditions.get(0);
            ...
            if (planForPreconditionOpt.isPresent()) {
                ...
                var planAfterPrecondition = new LinkedList<Atom>(planForPreconditionOpt.get());
    ...
    public class Agent {
        ...
        private Atom perceiveAndLearnState(Collection<Atom> perceivedAtoms) {
            ...
            perceivedAtoms.parallelStream().forEach(pAtom -> {
                ...
                var perceivedCertainty = Certainty.of(pAtom.certainty().strength, AGENT_DEFAULT_PERCEPTION_COUNT);
                ...
                var learnedAtom = mem.learnAtom(currentPercept);
                ...
                if (learnedAtom.certainty().strength > 0.5 && learnedAtom.certainty().confidence() > 0.5) {
            ...
            if (presentFluentIds.isEmpty()) {
                var emptyState = mem.node("State:Empty", Certainty.TRUE, 0.8);
            ...
            var stateNode = mem.node(stateName, Certainty.TRUE, 0.9); // High STI for current state
            ...
            //     kb.learnLinkByIds(Link.Type.MEMBER, Certainty.TRUE, 0.7, stateNode.id(), fluentId);
            // }
        ...
        private Optional<Atom> selectAction(Game env) {
            var availableActions = env.getAvailableActions();
            ...
            if (goodActionNode!=null) {
                ...
                availableActions.parallelStream().forEach(action -> {
                    ...
                    var utility = results.stream()
                            .mapToDouble(r -> r.inferredAtom.certainty().strength * r.inferredAtom.certainty().confidence()) // Combine strength & confidence
        ...
        private void learnFromExperience(Atom currentStateAtom, double reward, long currentTime) {
            ...
            var sequenceLink = mem.link(Link.Type.SEQUENCE, Certainty.TRUE, 0.8, null, previousStateAtom, lastActionAtom);
            var transitionCertainty = Certainty.of(1.0, AGENT_DEFAULT_LEARNING_COUNT); // Observed transition
            ...
            var rewardCertainty = Certainty.of(normalizedRewardStrength, AGENT_DEFAULT_LEARNING_COUNT * 0.5); // Less count than transition
        ...
        private boolean checkGoalAchieved() {
            ...
            return results.stream().anyMatch(r -> r.inferredAtom.certainty().strength > 0.9 && r.inferredAtom.certainty().confidence() > 0.8);
        }
    }
    ...
    public abstract static sealed class Atom permits Node, Link {
        ...
        @SuppressWarnings("unused") private volatile double sti; // STI
        @SuppressWarnings("unused") private volatile double lti;  // LTI
        @SuppressWarnings("unused") private volatile long lastAccessTime;


        protected Atom(String id, Certainty certainty) {
        ...
        // Internal setter used by KB revision
        final void setCertainty(Certainty certainty) {
        ...
        public final double getCurrentImportance(long currentTime) {
            ...
            return combinedImportance * certainty.confidence();
        }
         ...
         */
        public abstract Atom withCertainty(Certainty newCertainty);
        ...
        public String toString() {
            return String.format("%s %s", id, certainty);
    ...
    public static sealed class Node extends Atom {
        ...
        public Node(String name, Certainty certainty) {
        ...
        public Atom withCertainty(Certainty newCertainty) {
            ...
            // Copy importance state - assumes temporary copy, KB manages learned atom's state
        ...
        @Override
        public String toString() {
            return String.format("%s %s", name, certainty());
     ...
     */
    public record Certainty(double strength, double count) {
        public static final Certainty DEFAULT = Certainty.of(0.5, 0.0); // Default prior: ignorance
        public static final Certainty TRUE = Certainty.of(1, 10.0); // Strong positive evidence
        public static final Certainty FALSE = Certainty.of(0.0, 10.0); // Strong negative evidence
        public static final Certainty UNKNOWN = Certainty.of(0.5, 0.1); // Explicitly unknown/low evidence
         ...
         */
        public static Certainty of(double strength, double count) {
            return new Certainty(unitize(strength), Math.max(0, count));
        }
        ...
        public double confidence() {
            return count / (count + CERTAINTY_DEFAULT_SENSITIVITY);
        }
         ...
         */
        public Certainty merge(Certainty other) {
            var oc = other.count;
            if (oc == 0) return this;
            var tc = this.count;
            ...
            return Certainty.of(mergedStrength, totalCount);
        }

        @Override
        public String toString() {
            return String.format("<s:%.3f, c:%.2f, conf:%.3f>", strength, count, confidence());
        }
    }
    ...
    public record Time(long start, long end, boolean isRelative) {
        ...
        public static Time merge(Link a, Link b) {
            return Time.merge(a.time().orElse(null), b.time().orElse(null));
        }
     ...
     */
    public record Answer(Bindings bindings, Atom inferredAtom) {
    }

    /**
     * Represents variable bindings during unification. Immutable.
     */
    public record Bindings(Map<String, String> map) {
        public static final Bindings EMPTY_BINDINGS = new Bindings(Collections.emptyMap());

        public Bindings {
            map = Map.copyOf(map);
        } // Ensure immutability

        public Optional<String> getBinding(String varId) {
            return Optional.ofNullable(map.get(varId));
        }

        /**
         * Creates new bindings extended with one more mapping. Returns empty Optional if inconsistent.
         */
        public Optional<Bindings> extend(String varId, String valueId) {
            var mk = map.get(varId);
         ...
         */
        public Optional<Bindings> merge(Bindings other) {
            ...
            return Optional.of(new Bindings(mergedMap));
        }
    ...
    public static class Unification {
         ...
         */
        public Optional<Bindings> unifyAtomId(String patternId, String instanceId, Bindings bindings) {
            var pId = bindings.getBinding(patternId).orElse(patternId);
            var iId = bindings.getBinding(instanceId).orElse(instanceId);
         ...
         */
        public Atom substitute(Atom pattern, Bindings bindings) {
            ...
            if (pattern instanceof Link link) {
                ...
                if (changed) {
                    ...
                    var newLink = new Link(link.type, newTargets, link.certainty(), link.time().orElse(null));
        ...
        private String followBindingChain(String id, Bindings bindings) {
            var currentId = id;
    ...
    static class BasicGridWorld implements Game {
        ...
        private Node getPositionNode(int x, int y) {
            ...
            return positionNodes.computeIfAbsent(name, k -> cle.getOrCreateNode(k));
        }
        ...
        public Percept perceive() {
            ...
            var atLocationEval = cle.learnLink(Link.Type.EVALUATION, Certainty.TRUE, 0.8, atLocationPred, agentNode, currentPosNode);
            // 2. Create the outer HoldsAt link: HoldsAt(InnerLink, Time)
            Atom agentAtLink = cle.learnLink(Link.Type.HOLDS_AT, Certainty.TRUE, 0.9,
        ...
        @Override
        public Act executeAction(Atom actionSchema) {
            steps++;
