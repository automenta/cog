...
/**
 * Cog - Cognitive Logic Engine (Synthesized Hyperon-Inspired Iteration v6.1)
 * <p>
 ...
 * - **Bridged Primitives (`Is`):** Connect MeTTa symbols to Java code/data for I/O, math, environment interaction, and JVM control via {@link Jvm}.
 ...
 * - **JVM Integration (`Jvm`):** Allows MeTTa code to introspect and manipulate the Java environment (object creation, method calls, field access, proxies).
...
public final class Cog {
    ...
    private static final double PRI_INITIAL_STI = 0.2; // Excluded (Budget)
    private static final double PRI_INITIAL_LTI_FACTOR = 0.1; // Excluded (Budget)
    private static final double PRI_STI_DECAY_RATE = 0.08; // Excluded (Budget)
    private static final double PRI_LTI_DECAY_RATE = 0.008; // Excluded (Budget)
    private static final double PRI_STI_TO_LTI_RATE = 0.02; // Excluded (Budget)
    private static final double PRI_BOOST_ON_ACCESS = 0.08; // Excluded (Budget)
    private static final double PRI_BOOST_ON_REVISION_MAX = 0.5; // Excluded (Budget)
    private static final double PRI_BOOST_ON_GOAL_FOCUS = 0.95; // Excluded (Budget)
    private static final double PRI_BOOST_ON_PERCEPTION = 0.75; // Excluded (Budget)
    private static final double PRI_MIN_FORGET_THRESHOLD = 0.015; // Candidate for MeTTa config (tune forgetting aggression)
    ...
    private @Nullable Game environment; // Holds the current environment for agent Is functions

    /**
     * Initializes all components of the Cog engine.
     */
    public Cog() {
    ...
    /**
     * Helper to clamp a double value between 0.0 and 1.0.
     */
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("Memory size after maintenance: " + sizeAfter + " (Target reduction if size > " + FORGETTING_MAX_MEM_SIZE_TRIGGER + ")");
    ...
    public void runAgent(Game env, Atom goal, int maxCycles) {
        this.environment = env; // Make environment accessible to Is functions via env()
        try {
            agent.run(env, goal, maxCycles); // Delegate to agent's run method
        } finally {
            this.environment = null; // Clear reference afterwards
        }
    }
    ...
    public sealed interface Atom {
        ...
        // Convenience casting methods (use with caution or instanceof checks)
        ...
        @Override boolean equals(Object other); // Must be implemented based on id()
        @Override int hashCode(); // Must be implemented based on id()
        @Override String toString(); // Human-readable representation (often same as id())
    }
    ...
    public record Sym(String name) implements Atom {
        ...
        public static Sym of(String name) { return SYMBOL_CACHE.computeIfAbsent(name, Sym::new); } // Canonical instances

        @Override public String id() { return name; }
        @Override public AtomType type() { return AtomType.SYM; }
    ...
    public record Var(String name) implements Atom {
        public Var { if (name.startsWith(VAR_PREFIX)) throw new IllegalArgumentException("Var name excludes prefix"); } // Validate name

        @Override public String id() { return VAR_PREFIX + name; }
        @Override public AtomType type() { return AtomType.VAR; }
    ...
    public record Is<T>(String id, @Nullable T value, @Nullable Function<List<Atom>, Optional<Atom>> fn) implements Atom {
        ...
        public Is(String name, Function<List<Atom>, Optional<Atom>> fn) { this(name, null, fn); } // ID is the function name
        ...
        private static <T> String deriveId(T value) {
            ...
            else if (value instanceof Double d) { typeName = "Double"; valuePart = d.toString(); } // Use Double for consistency
        ...
        public boolean isFn() { return fn != null; } // Check if this Is atom represents a function
        public Optional<Atom> apply(List<Atom> args) { return isFn() ? fn.apply(args) : empty(); } // Apply the function if present
        ...
        @Override public int hashCode() { return id.hashCode(); }
        @Override public boolean equals(Object o) { return this == o || (o instanceof Is<?> ga && id.equals(ga.id)); } // Equality based on ID
    }
    ...
    public record Value(Truth truth, Pri pri, Time access) {
        ...
        public Value boost(double amount, long now) { return new Value(truth, pri.boost(amount), new Time(now)); }
        ...
        public double getCurrentPri(long now) {
            ...
            var recencyFactor = Math.exp(-timeSinceAccess / (double)(FORGETTING_CHECK_INTERVAL_MS * 3));
    ...
    public record Truth(double strength, double count) {
        ...
        public Truth { strength = unitize(strength); count = Math.max(0, count); } // Enforce bounds
    ...
    public record Pri(double sti, double lti) { // Kept in Java for performance
        public static final Pri DEFAULT = new Pri(PRI_INITIAL_STI, PRI_INITIAL_STI * PRI_INITIAL_LTI_FACTOR);

        public Pri { sti = unitize(sti); lti = unitize(lti); } // Enforce bounds [0,1]
        ...
        public double getCurrent(double recencyFactor) { return sti * recencyFactor * 0.6 + lti * 0.4; } // Weighted mix of STI (recent) and LTI (stable)
    ...
    public static class Mem {
        ...
        private final ConcurrentMap<Atom, AtomicReference<Value>> storage = new ConcurrentHashMap<>(1024); // Atom -> Metadata Ref
        ...
        public <A extends Atom> A add(A atom) {
            var canonicalAtom = (A) atomsById.computeIfAbsent(atom.id(), id -> atom); // Ensure canonical instance
            long now = timeSource.get();
            var valueRef = storage.computeIfAbsent(canonicalAtom, k -> { // Initialize if new
            ...
            valueRef.updateAndGet(v -> v.boost(PRI_BOOST_ON_ACCESS * 0.1, now)); // Slight boost on add/access
        ...
        public Optional<Value> value(Atom atom) { return getAtom(atom).flatMap(key -> Optional.ofNullable(storage.get(key))).map(AtomicReference::get); }
        ...
        public void updateValue(Atom atom, Function<Value, Value> updater) {
            ...
            if (valueRef != null) {
                ...
                valueRef.updateAndGet(current -> {
                    ...
                    var boost = (confDiff > TRUTH_REVISION_CONFIDENCE_THRESHOLD) ? PRI_BOOST_ON_REVISION_MAX * confDiff : 0.0;
        ...
        public List<Answer> query(Atom pattern) {
            ...
            boost(queryPattern, PRI_BOOST_ON_ACCESS * 0.2); // Boost the query pattern itself
        ...
        private Stream<Atom> getCandidateStream(Atom queryPattern) {
            return switch (queryPattern) {
                ...
                case Expr pExpr when pExpr.head() != null && !(pExpr.head() instanceof Var) ->
                    Stream.concat(
                        indexByHead.getOrDefault(pExpr.head(), Set.of()).stream() // Get indexed IDs
                            .map(this::getAtom).flatMap(Optional::stream), // Fetch corresponding Atoms
                ...
                case Var v -> storage.keySet().stream();
                ...
                default -> {
                    ...
                    Stream<Atom> broadScan = (queryPattern instanceof Expr pExpr && pExpr.head() instanceof Var) ?
                         storage.keySet().stream().filter(a -> a.type() == AtomType.EXPR) : Stream.empty();
        ...
        synchronized void decayAndForget() {
            ...
            final var currentMaxMemTrigger = FORGETTING_MAX_MEM_SIZE_TRIGGER;
            ...
            List<Atom> candidatesForRemoval = new ArrayList<>(initialSize / 10); // Preallocate estimate
            var decayCount = 0;
            ...
            if (!candidatesForRemoval.isEmpty() && (memoryPressure || needsTrimming)) {
                ...
                var removalTargetCount = (memoryPressure || needsTrimming) ? Math.max(0, initialSize - targetSize) : candidatesForRemoval.size();
                var actuallyRemoved = 0;
                for (int i = 0; i < candidatesForRemoval.size() && actuallyRemoved < removalTargetCount; i++) {
                    var atomToRemove = candidatesForRemoval.get(i);
    ...
    private static class MettaParser {
        ...
        private Atom parseSymbolOrIs(String text) {
            return switch (text) {
                ...
                default -> { // Try parsing as number, fallback to symbol
                    try { yield cog.IS(Double.parseDouble(text)); } // Use Double for all numbers
                    catch (NumberFormatException e) { yield cog.S(text); }
                }
            };
        }
        ...
        private List<Token> tokenize(String text) {
            ...
            var line = 1; var col = 1; var i = 0; final var len = text.length();
            while (i < len) {
                ...
                if (Character.isWhitespace(c)) { if (c == '\n') { line++; col = 1; } else col++; i++; }
                ...
                else if (c == ';') { while (i < len && text.charAt(i) != '\n') i++; } // Skip comment until newline or EOF
                    ...
                    if (varName.length() <= VAR_PREFIX.length()) throw new MettaParseException("Invalid var name '" + varName + "' at line " + line + ":" + startCol);
        ...
        private Atom parseAtomFromTokens(PeekableIterator<Token> it) {
            ...
            return switch (token.type) {
                ...
                case STRING -> cog.IS(unescapeString(token.text.substring(1, token.text.length() - 1))); // Create Is<String>
        ...
        private Expr parseExprFromTokens(PeekableIterator<Token> it) {
            ...
            while (true) {
                ...
                if (nextToken.type == TokenType.RPAREN) { it.next(); return cog.E(children); } // End of expression
        ...
        public List<Atom> parseAll(String text) {
            ...
            var it = new PeekableIterator<>(tokens.iterator());
            while (it.hasNext()) { results.add(parseAtomFromTokens(it)); }
            return results;
        }
    ...
    private static class Core {
        ...
        Core(Cog cog) {
            ...
            SYMBOL_EQ = cog.S("="); SYMBOL_COLON = cog.S(":"); SYMBOL_ARROW = cog.S("->");
            SYMBOL_TYPE = cog.S("Type"); SYMBOL_TRUE = cog.S("True"); SYMBOL_FALSE = cog.S("False");
            SYMBOL_SELF = cog.S("Self"); SYMBOL_NIL = cog.S("Nil");
            ...
            Stream.concat(PROTECTED_SYMBOLS.stream(), Stream.of("Z", "S", "add")) // Include Peano symbols
                  ...
                  .forEach(sym -> {
                      ...
                      cog.mem.boost(sym, 1.0); // Max boost
                  });
        }
        ...
        private static Optional<Atom> applyNumOp(List<Atom> args, Cog cog, BinaryOperator<Double> op) {
            ...
            var n1Opt = numValue(args.get(0), cog); var n2Opt = numValue(args.get(1), cog);
        ...
        private static Optional<Atom> applyNumPred(List<Atom> args, Cog cog, BiPredicate<Double, Double> op) {
            if (args.size() != 2) return empty();
            var n1Opt = numValue(args.get(0), cog); var n2Opt = numValue(args.get(1), cog);
            return n1Opt.flatMap(n1 -> n2Opt.map(n2 -> op.test(n1, n2) ? SYMBOL_TRUE : SYMBOL_FALSE));
        }
        ...
        private void initIsFn() {
            ...
            cog.IS("remove-atom", args -> Optional.of((!args.isEmpty() && cog.mem.removeAtomInternal(args.getFirst())) ? SYMBOL_TRUE : SYMBOL_FALSE)); // Remove atom
            ...
            cog.IS("If", args -> (args.size() != 3) ? empty() :
                cog.evalBest(args.getFirst()) // Evaluate condition
                    .map(condResult -> condResult.equals(SYMBOL_TRUE) ? args.get(1) : args.get(2)) // Return unevaluated then/else branch
            );
            cog.IS("Let", args -> (args.size() != 2 || !(args.get(0) instanceof Expr b) || !b.head().equals(SYMBOL_EQ) || b.children().size() != 3 || !(b.children().get(1) instanceof Var v)) ? empty() :
            ...
            cog.IS("IsEmpty", args -> args.isEmpty() ? Optional.of(SYMBOL_TRUE) :
                cog.evalBest(args.getFirst()).map(res -> // Evaluate arg
                    (res.equals(SYMBOL_NIL) || (res instanceof Is<?> is && is.value() instanceof List<?> l && l.isEmpty())) ? SYMBOL_TRUE : SYMBOL_FALSE
                ));
            ...
            cog.IS("RandomChoice", args -> args.stream().findFirst() // Takes one argument (the list)
                ...
                .or(()-> args.stream().findFirst().flatMap(a -> cog.evalBest(a)).flatMap(Core::listValue)) // Or evaluate arg then get List<Atom>
                .filter(list -> !list.isEmpty())
                .map(list -> list.get(RandomGenerator.getDefault().nextInt(list.size()))) // Choose random element
                .or(() -> Optional.of(SYMBOL_NIL)) // Return Nil if list empty or arg invalid
            );
    ...
    /** Interface for the external environment the Agent interacts with. */
    public interface Game {
        ...
        boolean isRunning(); // Check if the environment simulation should continue
    }

    /** Result of an agent's action execution in the environment. */
    public record Act(List<Atom> newPercepts, double reward) {}

    /** Example minimal Game environment for testing agent logic. */
    static class SimpleGame implements Game {
        ...
        SimpleGame(Cog c) {
            ...
            posA = c.S("Pos_A"); posB = c.S("Pos_B"); posGoal = c.S("AtGoal");
            moveAtoB = c.S("Move_A_B"); moveBtoGoal = c.S("Move_B_Goal"); moveOther = c.S("Move_Other");
            statePred = c.S("State"); selfSym = c.S("Self");
        ...
        @Override public List<Atom> actions(Cog cog, Atom currentStateAtom) {
            ...
            return List.of(moveOther); // Only 'other' if at goal or invalid state
        }
        ...
        @Override public Act exe(Cog cog, Atom actionSymbol) {
            ...
            if (currentStateSymbol.equals(posA) && actionSymbol.equals(moveAtoB)) {
                currentStateSymbol = posB; reward = 0.1; logMsg = "Moved A -> B";
            } else if (currentStateSymbol.equals(posB) && actionSymbol.equals(moveBtoGoal)) {
                currentStateSymbol = posGoal; reward = 1.0; logMsg = "Moved B -> Goal!";
            } else if (actionSymbol.equals(moveOther)) {
                reward = -0.2; logMsg = "Executed 'Move_Other'";
            } else {
                reward = -0.5; logMsg = "Invalid action: " + actionSymbol + " from " + currentStateSymbol; // Penalty
            }
    ...
    private static class Jvm {
        ...
        private static final Set<String> ALLOWED_CLASSES = Set.of(
                ...
                "dumb.hyp6.Cog", "dumb.hyp6.Cog$Atom", // Allow access to Cog internals if needed cautiously
        ...
        @Nullable private Object meTTaToJava(@Nullable Atom atom) {
            return switch (atom) {
                case null -> null;
                case Atom a when a.equals(SYMBOL_NIL) -> null; // Map Nil to null
        ...
        private Atom javaToMeTTa(@Nullable Object obj) {
            if (obj == null) return SYMBOL_NIL; // Map null back to Nil
            // Avoid double wrapping if Java method already returns an Atom
            if (obj instanceof Atom a) return a;
            // Canonical True/False for booleans
            if (obj instanceof Boolean b) return b ? SYMBOL_TRUE : SYMBOL_FALSE;
            // Use IS to wrap Java objects, ensuring Numbers become Is<Double> for consistency
            if (obj instanceof Number n) return cog.IS(n.doubleValue());
        ...
        private Class<?> getArgType(@Nullable Atom atom) {
            ...
            if (obj == null) return Object.class; // Use Object for null args
            ...
            // Otherwise, use the object's actual class
            return obj.getClass();
        }
        // Finds a class safely, checking allow list.
        @Nullable private Class<?> findClass(String className) {
            if (!isClassAllowed(className)) { System.err.println("JvmBridge Security: Class not allowed: " + className); return null; }
        ...
        @Nullable private Object attemptConversion(Object value, Class<?> targetType) {
            if (value == null) return null;
            // Basic numeric conversions from Double (common case from MeTTa)
            if (value instanceof Double d) {
                if (targetType == int.class || targetType == Integer.class) return d.intValue();
                if (targetType == long.class || targetType == Long.class) return d.longValue();
                if (targetType == float.class || targetType == Float.class) return d.floatValue();
                if (targetType == short.class || targetType == Short.class) return d.shortValue();
                if (targetType == byte.class || targetType == Byte.class) return d.byteValue();
            }
            ...
            System.err.println("JvmBridge Warning: Could not convert " + value.getClass().getSimpleName() + " to " + targetType.getSimpleName());
        ...
        public Optional<Atom> javaNew(List<Atom> args) {
            ...
            if (!(meTTaToJava(args.getFirst()) instanceof String className)) return Optional.of(cog.S("Error:JavaNew:ClassNameNotString"));
            ...
            try {
                ...
                if (constructor == null) return Optional.of(cog.S("Error:JavaNew:ConstructorNotFound:" + Arrays.toString(argTypes)));
        ...
        private Optional<Atom> invokeMethod(boolean isStatic, List<Atom> args) {
            int minArgs = isStatic ? 2 : 2; // Static: class, method; Instance: instance, method
            ...
            if (isStatic) {
                if (!(meTTaToJava(args.get(0)) instanceof String className)) return Optional.of(cog.S("Error:Invoke:ClassNameNotString"));
                clazz = findClass(className);
                if (clazz == null) return Optional.of(cog.S("Error:Invoke:ClassNotFound:" + className));
                if (!(meTTaToJava(args.get(1)) instanceof String mName)) return Optional.of(cog.S("Error:Invoke:MethodNameNotString"));
                methodName = mName;
                ...
                if (!isClassAllowed(clazz.getName())) return Optional.of(cog.S("Error:Invoke:ClassNotAllowed:" + clazz.getName()));
                if (!(meTTaToJava(args.get(1)) instanceof String mName)) return Optional.of(cog.S("Error:Invoke:MethodNameNotString"));
                methodName = mName;
                methodArgStartIndex = 2;
            }

            if (!isMemberAllowed(clazz, methodName)) return Optional.of(cog.S("Error:Invoke:MemberNotAllowed:" + methodName));
            ...
            try {
                ...
                if (method == null) {
                    ...
                    if (isStatic && methodArgs.isEmpty()) { return accessStaticField(clazz, methodName); }
                    return Optional.of(cog.S("Error:Invoke:MethodNotFound:" + methodName + Arrays.toString(argTypes)));
                }

                // Get MethodHandle using lookup for potentially better performance
                MethodHandle methodHandle = isStatic ? lookup.unreflect(method) : lookup.unreflect(method);
                // MethodHandles require exact type matching, adjust args if needed (e.g., Double -> int)
                Object[] adjustedArgs = adjustArguments(javaArgs, method.getParameterTypes());

                // Invoke the method handle
                Object result;
                if (isStatic) { result = methodHandle.invokeWithArguments(adjustedArgs); }
                else { result = methodHandle.bindTo(target).invokeWithArguments(adjustedArgs); } // Bind instance for virtual call
                ...
                if (e instanceof InvocationTargetException ite && ite.getCause() != null) ite.getCause().printStackTrace(); // Unwrap target exception
        ...
        private Optional<Atom> accessStaticField(Class<?> clazz, String fieldName) {
            try {
                var field = clazz.getField(fieldName);
                if (!java.lang.reflect.Modifier.isStatic(field.getModifiers())) return Optional.of(cog.S("Error:Invoke:FieldNotStatic:" + fieldName));
                if (!isMemberAllowed(clazz, fieldName)) return Optional.of(cog.S("Error:Invoke:MemberNotAllowed:" + fieldName));
                return Optional.of(javaToMeTTa(field.get(null))); // Get static field value
            } catch (NoSuchFieldException e) { return Optional.of(cog.S("Error:Invoke:MethodOrFieldNotFound:" + fieldName)); } // Combined error msg
            catch (IllegalAccessException e) { return Optional.of(cog.S("Error:Invoke:FieldAccessDenied:" + fieldName)); }
        }

        // Helper to find constructor reflectively, handling type compatibility.
        @Nullable private java.lang.reflect.Constructor<?> findConstructor(Class<?> clazz, Class<?>[] argTypes) {
            try { return clazz.getConstructor(argTypes); } // Try exact match first
                ...
                for (var constructor : clazz.getConstructors()) { // Check all public constructors
                    if (constructor.getParameterCount() == argTypes.length) {
                        if (parametersMatch(constructor.getParameterTypes(), argTypes)) return constructor;
                    }
                }
        ...
        @Nullable private Method findMethod(Class<?> clazz, String methodName, Class<?>[] argTypes) {
            try { return clazz.getMethod(methodName, argTypes); } // Try exact match first
            catch (NoSuchMethodException e) {
        ...
        private Object[] adjustArguments(Object[] args, Class<?>[] formalTypes) {
            ...
            Object[] adjusted = new Object[args.length];
        ...
        public Optional<Atom> javaField(List<Atom> args) {
            ...
            var targetOrClass = meTTaToJava(targetOrClassAtom);
            if (!(meTTaToJava(args.get(1)) instanceof String fieldName)) return Optional.of(cog.S("Error:JavaField:FieldNameNotString"));
            ...
            else { return Optional.of(cog.S("Error:JavaField:InvalidTarget")); }
            ...
            try {
                ...
                if (isStatic != java.lang.reflect.Modifier.isStatic(field.getModifiers())) return Optional.of(cog.S("Error:JavaField:StaticMismatch"));

                if (args.size() == 2) { // Get field
                    return Optional.of(javaToMeTTa(field.get(instance))); // instance is null for static
                } else { // Set field
                    var valueToSetAtom = args.get(2);
                    var valueToSet = meTTaToJava(valueToSetAtom);
                    Object adjustedValue = valueToSet;
            ...
            } catch (NoSuchFieldException e) { return Optional.of(cog.S("Error:JavaField:NotFound:" + fieldName)); }
            catch (IllegalAccessException e) { return Optional.of(cog.S("Error:JavaField:AccessDenied:" + fieldName)); }
            catch (Exception e) { System.err.println("JvmBridge Error: Field operation failed: " + e); return Optional.of(cog.S("Error:JavaField:OperationFailed:" + e.getClass().getSimpleName())); }
        }
        ...
        public Optional<Atom> javaProxy(List<Atom> args) {
            ...
            if (!(meTTaToJava(args.get(0)) instanceof String interfaceName)) return Optional.of(cog.S("Error:JavaProxy:InterfaceNameNotString"));
            var handlerExprTemplate = args.get(1); // MeTTa atom defining the handler logic

            var iface = findClass(interfaceName);
            if (iface == null || !iface.isInterface()) return Optional.of(cog.S("Error:JavaProxy:InvalidInterface:" + interfaceName));
            ...
            try {
                ...
                return Optional.of(javaToMeTTa(proxyInstance)); // Wrap proxy in Is<>
            } catch (IllegalArgumentException e) { System.err.println("JvmBridge Error: Proxy creation failed: " + e); return Optional.of(cog.S("Error:JavaProxy:CreationFailed")); }
            catch (Exception e) { System.err.println("JvmBridge Error: Unexpected proxy error: " + e); return Optional.of(cog.S("Error:JavaProxy:UnexpectedError")); }
        }
        ...
        private InvocationHandler createInvocationHandler(Atom handlerExprTemplate) {
            ...
            return (proxy, method, methodArgs) -> {
                ...
                var proxyAtom = javaToMeTTa(proxy); // Optional: Represent proxy instance itself
                ...
                Optional<Atom> mettaResultOpt = cog.evalBest(exprToEval);

                // 4. Convert the MeTTa result back to the expected Java type
                var javaResult = mettaResultOpt.map(this::meTTaToJava).orElse(null);
                ...
                if (javaResult == null) {
                    if (returnType.isPrimitive()) throw new MettaProxyException("MeTTa handler returned null/Nil for primitive return type " + returnType.getName() + " in method " + methodNameAtom);
        ...
        private List<Atom> evalRecursive(Atom atom, int depth, Set<String> visitedInPath) {
            ...
            try {
                switch (atom) {
                    ...
                    case Expr expr -> {
                        boolean appliedRuleOrFn = false; // Track if a rewrite or execution happened

                        // Strategy 2a: Specific equality rule `(= <expr> $result)`
                        var resultVarSpecific = cog.V("evalResS" + depth); // Unique var name
                        Atom specificQuery = cog.E(SYMBOL_EQ, expr, resultVarSpecific);
                        ...
                        if (!appliedRuleOrFn || combinedResults.size() < INTERPRETER_MAX_RESULTS / 2) {
                            var pVar = cog.V("p" + depth); var tVar = cog.V("t" + depth);
                            Atom generalQuery = cog.E(SYMBOL_EQ, pVar, tVar);
                            ...
                            for (var ruleMatch : ruleMatches) {
                                ...
                                if (!combinedResults.isEmpty()) appliedRuleOrFn = true; // Mark rule applied if unification succeeded
                                if (combinedResults.size() >= INTERPRETER_MAX_RESULTS) break;
                            }
                        }
                        ...
                        if (/*!appliedRuleOrFn ||*/ combinedResults.isEmpty()) { // Check combinedResults too
                            ...
                            Atom currentHead = expr.head();
                            List<Atom> currentTail = expr.tail();

                            // Evaluate Head (if exists)
                            Atom newHead = currentHead;
                            if (currentHead != null) {
    ...
    public class Agent {
        ...
        public Agent(Cog cog) {
            ...
            Stream.of("AgentStep", "AgentPerceive", "AgentSelectAction", "AgentExecute", "AgentLearn", "CheckGoal", "UpdateUtility", "Act", "Utility", "First", "FirstNumOrZero", "RandomChoice")
                  ...
                  .forEach(s -> { cog.mem.updateTruth(s, Truth.TRUE); cog.mem.boost(s, 0.8); }); // Boost agent-specific concepts
        }
        ...
        public void run(Game env, Atom goalPattern, int maxCycles) {
            ...
            for (int cycle = 0; cycle < maxCycles && env.isRunning(); cycle++) {
                ...
                boolean goalMet = cog.evalBest(goalCheckExpr).filter(SYMBOL_TRUE::equals).isPresent();

                if (goalMet) {
                    System.out.println("*** Agent: Goal Achieved (evaluated CheckGoal as True)! ***");
                    break; // Exit loop successfully
                }
                if (!env.isRunning()) { // Check if environment stopped simulation (e.g., game over)
                    System.out.println("--- Agent Run Halted (Environment Stopped) ---");
                    break;
                }
            ...
            // --- Final Status Report ---
            boolean finalGoalCheck = cog.evalBest(cog.E(cog.S("CheckGoal"), SELF_SYM, goalPattern)).filter(SYMBOL_TRUE::equals).isPresent();
            if (!env.isRunning() && !finalGoalCheck) System.out.println("--- Agent Run Finished (Environment Stopped, Goal Not Met) ---");
            else if (!finalGoalCheck) System.out.println("--- Agent Run Finished (Max Cycles Reached, Goal Not Met) ---");
    ...
    public static class TestClass {
        ...
        public static String staticField = "StaticInitial"; // Public static field

        public TestClass() {} // Default constructor needed for JavaNew without args
