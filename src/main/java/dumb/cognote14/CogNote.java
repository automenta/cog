...
public class CogNote {
    ...
    public static final String
        KIF_OPERATOR_IMPLIES = "=>",
        KIF_OPERATOR_EQUIV = "<=>",
        KIF_OPERATOR_AND = "and",
        KIF_OPERATOR_OR = "or",
        KIF_OPERATOR_EXISTS = "exists",
        KIF_OPERATOR_FORALL = "forall",
        KIF_OPERATOR_EQUAL = "=",
        KIF_OPERATOR_NOT = "not";
    ...
    private static final String
        ID_PREFIX_RULE = "rule",
        ID_PREFIX_FACT = "fact",
        ID_PREFIX_SKOLEM_FUNC = "skf_",
        ID_PREFIX_SKOLEM_CONST = "skc_",
        ID_PREFIX_ENTITY = "entity_",
        ID_PREFIX_NOTE = "note-",
        ID_PREFIX_LLM_ITEM = "llm_",
        ID_PREFIX_INPUT_ITEM = "input_",
        ID_PREFIX_TEMP_ITEM = "temp_";
    ...
    private static final String
        CALLBACK_ASSERT_ADDED = "assert-added",
        CALLBACK_ASSERT_INPUT = "assert-input",
        CALLBACK_ASSERT_RETRACTED = "assert-retracted",
        CALLBACK_EVICT = "evict",
        CALLBACK_LLM_RESPONSE = "llm-response";
    ...
    private static final int
        KB_SIZE_THRESHOLD_WARN_PERCENT = 90,
        KB_SIZE_THRESHOLD_HALT_PERCENT = 98,
        COMMIT_QUEUE_CAPACITY = 1024 * 1024,
        TASK_QUEUE_CAPACITY = 1024 * 1024,
        QUEUE_SIZE_THRESHOLD_WARN = TASK_QUEUE_CAPACITY / 2,
        QUEUE_SIZE_THRESHOLD_HALT = TASK_QUEUE_CAPACITY * 9 / 10,
    ...
    final ReasonerEngine reasonerEngine;
    ...
    volatile String reasonerStatus = "Idle";
    ...
    public CogNote(int port, int capacity, boolean broadcastInput, String llmUrl, String llmModel, SwingUI ui) {
        ...
        this.knowledgeBase = new KnowledgeBase(capacity, this::callback);
        ...
        System.out.printf("Reasoner config: Port=%d, KBSize=%d, BroadcastInput=%b, LLM_URL=%s, LLM_Model=%s, MaxDepth=%d%n",
                port, capacity, broadcastInputAssertions, llmApiUrl, llmModel, MAX_DERIVATION_DEPTH);
    }
    ...
    private void broadcastMessage(String type, SwingUI.AssertionViewModel llmItem) {
        if (!CALLBACK_LLM_RESPONSE.equals(type) || llmItem.noteId() == null) return;
        var message = String.format("llm-response %s [%s] {type:%s, content:\"%s\"}",
                llmItem.noteId(), llmItem.id(), llmItem.displayType(), llmItem.content().replace("\"", "\\\""));
        safeBroadcast(message);
    }
    ...
    public void loadExpressionsFromFile(String filename) throws IOException {
        ...
        var counts = new long[]{0, 0, 0}; // lines, blocks, queued
    ...
    void queueExpressionFromSource(KifTerm term, String sourceId, @Nullable String sourceNoteId) {
    ...
    private void handleForallInput(KifList forallExpr, String sourceId, @Nullable String sourceNoteId) {
        ...
        if (body.getOperator().filter(op -> op.equals(KIF_OPERATOR_IMPLIES) || op.equals(KIF_OPERATOR_EQUIV)).isPresent()) {
            ...
            submitPotentialAssertion(new PotentialAssertion(forallExpr, pri, Set.of(), sourceId, false, false, false, sourceNoteId, AssertionType.UNIVERSAL, List.copyOf(variables), 0));
        } else {
    ...
    public void stopReasoner() {
        ...
        http.executor().filter(ExecutorService.class::isInstance).map(ExecutorService.class::cast)
    ...
    public void setPaused(boolean pause) {
        if (paused == pause || !running) return;
        paused = pause;
    ...
    void submitPotentialAssertion(PotentialAssertion pa) {
        if (!running || pa == null || (pa.kif() instanceof KifList kl && isTrivial(kl))) return;

        var isDirectInputSource = !(pa.sourceId().startsWith(ID_PREFIX_RULE) || pa.sourceId().startsWith(ID_PREFIX_FACT));
        if (isDirectInputSource && pa.sourceNoteId() != null) {
            var inputAssertion = new Assertion(generateId(ID_PREFIX_INPUT_ITEM), pa.kif(), pa.pri(), System.currentTimeMillis(), pa.sourceNoteId(),
                    pa.support(), pa.derivedType(), pa.isEquality(), pa.isOrientedEquality(),
    ...
    void callback(String type, Object payload) {
        switch (payload) {
            ...
            case SwingUI.AssertionViewModel llmItem when CALLBACK_LLM_RESPONSE.equals(type) ->
                    invokeLlmCallbacks(type, llmItem);
            default -> System.err.println("Warning: Unknown callback payload type: " + payload.getClass());
        }
        if (!reasonerStatus.matches("Cleared|Stopping|Stopped")) {
            updateUIStatus();
        }
    }

    private void invokeAssertionCallbacks(String type, Assertion a) {
        var primaryNoteId = a.sourceNoteId();
        var commonAncestorNoteId = reasonerEngine.findCommonSourceNodeId(a.support());
        var displayNoteId = primaryNoteId != null ? primaryNoteId : commonAncestorNoteId;

        if (primaryNoteId != null) {
            if (CALLBACK_ASSERT_ADDED.equals(type)) {
                noteIdToAssertionIds.computeIfAbsent(primaryNoteId, _ -> ConcurrentHashMap.newKeySet()).add(a.id());
            } else if (CALLBACK_ASSERT_RETRACTED.equals(type) || CALLBACK_EVICT.equals(type)) {
                noteIdToAssertionIds.computeIfPresent(primaryNoteId, (_, set) -> {
                    set.remove(a.id());
        ...
        if (swingUI != null && swingUI.isDisplayable()) {
            swingUI.handleReasonerCallback(type, a, displayNoteId);
        }

        if (CALLBACK_ASSERT_ADDED.equals(type)) {
            callbackRegistrations.forEach(reg -> {
                try {
                    ofNullable(Unifier.match(reg.pattern, a.kif(), Map.of()))
    ...
    private void invokeLlmCallbacks(String type, SwingUI.AssertionViewModel llmItem) {
        if (swingUI != null && swingUI.isDisplayable()) {
            swingUI.handleReasonerCallback(type, llmItem, llmItem.noteId());
        }
        if (broadcastInputAssertions) {
            broadcastMessage(type, llmItem);
        }
    }
    ...
    void updateUIStatus() {
        if (swingUI != null && swingUI.isDisplayable()) {
            var statusText = String.format("KB: %d/%d | Tasks: %d | Commits: %d | Rules: %d | Status: %s",
                    knowledgeBase.getAssertionCount(), capacity,
                    reasonerEngine.getTaskQueueSize(), reasonerEngine.getCommitQueueSize(),
                    reasonerEngine.getRuleCount(), reasonerEngine.getCurrentStatus());
    ...
    private CompletableFuture<String> llmAsync(String prompt, String interactionType, String noteId) {
        ...
        }, http.executor().orElse(Executors.newVirtualThreadPerTaskExecutor()));
    }
    ...
    public CompletableFuture<String> text2kifAsync(String noteText, String noteId) {
        ...
        return llmAsync(finalPrompt, "KIF Generation", noteId)
                ...
                .whenComplete((cleanedKif, ex) -> {
                    if (ex == null && cleanedKif.trim().isEmpty())
                    ...
                    else if (!cleanedKif.trim().isEmpty())
    ...
    private Optional<String> extractLlmContent(JSONObject r) {
        return ofNullable(r.optJSONObject("message"))
                .map(m -> m.optString("content", null))
                .or(() -> ofNullable(r.optString("response", null)))
                .or(() -> ofNullable(r.optJSONArray("choices"))
                        .filter(Predicate.not(JSONArray::isEmpty))
                        .map(a -> a.optJSONObject(0))
                        .map(c -> c.optJSONObject("message"))
                        .map(m -> m.optString("content", null)))
                .or(() -> ofNullable(r.optJSONArray("results"))
                        .filter(Predicate.not(JSONArray::isEmpty))
                        .map(a -> a.optJSONObject(0))
                        .map(res -> res.optJSONObject("candidates"))
                        .map(cand -> cand.optJSONObject("content"))
                        .map(cont -> cont.optJSONArray("parts"))
                        .filter(Predicate.not(JSONArray::isEmpty))
                        .map(p -> p.optJSONObject(0))
                        .map(p -> p.optString("text", null)))
    ...
    sealed interface KifTerm permits KifAtom, KifVar, KifList {
        ...
        default boolean containsSkolemTerm() {
            return switch (this) {
                ...
                case KifList l -> l.getOperator().filter(op -> op.startsWith(ID_PREFIX_SKOLEM_FUNC)).isPresent() ||
                        l.terms.stream().anyMatch(KifTerm::containsSkolemTerm);
    ...
    static final class KifList implements KifTerm {
        ...
        Optional<String> getOperator() {
            return terms.isEmpty() || !(terms.getFirst() instanceof KifAtom(
                    var value
    ...
    static class PathIndex {
        ...
        private void findUnifiableRecursive(KifTerm queryTerm, PathNode indexNode, Set<String> candidates) {
            ...
            ofNullable(indexNode.children.get(PathNode.VAR_MARKER))
                    .ifPresent(varNode -> collectAllAssertionIds(varNode, candidates));

            if (queryTerm instanceof KifList) {
                ofNullable(indexNode.children.get(PathNode.LIST_MARKER))
        ...
        private void findInstancesRecursive(KifTerm queryPattern, PathNode indexNode, Set<String> candidates) {
            ...
            if (specificNode != null) {
                candidates.addAll(specificNode.assertionIdsHere);
                if (queryPattern instanceof KifList) {
                    collectAllAssertionIds(specificNode, candidates);
                }
            }
        }
        ...
        private void findGeneralizationsRecursive(KifTerm queryTerm, PathNode indexNode, Set<String> candidates) {
            ...
            ofNullable(indexNode.children.get(getIndexKey(queryTerm)))
                    .ifPresent(nextNode -> {
    ...
    static class KnowledgeBase {
        ...
        private final ConcurrentMap<KifAtom, Set<String>> universalIndex = new ConcurrentHashMap<>();
        ...
        private final PriorityBlockingQueue<String> groundEvictionQueue = new PriorityBlockingQueue<>(1024,
                Comparator.<String, Double>comparing(id -> ofNullable(assertionsById.get(id)).map(Assertion::pri).orElse(Double.MAX_VALUE))
                        .thenComparing(id -> ofNullable(assertionsById.get(id)).map(Assertion::timestamp).orElse(Long.MAX_VALUE)));
        ...
        Optional<Assertion> commitAssertion(PotentialAssertion pa) {
            ...
            try {
                ...
                boolean alreadyExistsOrSubsumed = switch (finalType) {
                ...
                switch (finalType) {
                    ...
                    case UNIVERSAL -> newAssertion.getReferencedPredicates().forEach(pred ->
        ...
        private Assertion retractAssertionWithCascadeInternal(String id, Set<String> retractedInThisCascade) {
            ...
            if (removed != null) {
                switch (removed.assertionType()) {
                    ...
                    case UNIVERSAL -> removed.getReferencedPredicates().forEach(pred ->
                ...
                ofNullable(assertionDependencies.remove(id))
                        .ifPresent(dependents ->
                                new HashSet<>(dependents)
                                        .forEach(depId -> retractAssertionWithCascadeInternal(depId, retractedInThisCascade))
                        );
        ...
        private boolean isSubsumedInternal(KifTerm term, boolean isNegated) {
            return pathIndex.findGeneralizations(term).stream()
                    ...
                    .filter(Objects::nonNull)
                    .filter(a -> a.assertionType() == AssertionType.GROUND || a.assertionType() == AssertionType.SKOLEMIZED)
                    .anyMatch(candidate -> candidate.isNegated() == isNegated
                            && Unifier.match(candidate.getEffectiveTerm(), term, Map.of()) != null);
        }
        ...
        private Optional<Assertion> findExactMatchInternal(KifList kif) {
            return pathIndex.findInstances(kif).stream()
                    ...
                    .findFirst()
                    .or(() -> assertionsById.values().stream()
                            .filter(a -> a.kif().equals(kif))
                            .findFirst());
        }
    ...
    static class ReasonerEngine {
        ...
        void start() {
            ...
            var workerCount = Math.max(MIN_INFERENCE_WORKERS, Runtime.getRuntime().availableProcessors());
        ...
        boolean removeRule(Rule rule) {
            if (rules.remove(rule)) {
                System.out.println("Removed rule: " + rule.id() + " " + rule.form().toKifString());
                return true;
            }
            return false;
        }
        ...
        private void handleApplyRewriteTask(InferenceTask task) {
            ...
            Unifier.rewrite(targetAssertion.kif(), ruleAssertion.kif().get(1), ruleAssertion.kif().get(2))
                    ...
                    .ifPresent(rewrittenList -> {
                        ...
                        submitPotentialAssertion(new PotentialAssertion(rewrittenList, calculateDerivedPri(support, task.pri()), support,
                                ruleAssertion.id(), isEq, isNeg, isOriented,
                                findCommonSourceNodeId(support),
                                derivedType, List.of(), newDepth));
                    });
        }
        ...
        private void processDerivedAssertion(Rule sourceRule, MatchResult matchResult) {
            ...
            if (simplifiedTerm instanceof KifList derivedList) {
                derivedList.getOperator().ifPresentOrElse(
                        op -> {
                            switch (op) {
                                case KIF_OPERATOR_AND ->
                                        processDerivedConjunction(sourceRule, derivedList, matchResult);
                        ...
                        () -> processDerivedStandard(sourceRule, derivedList, matchResult)
                );
        ...
        private void processDerivedForall(Rule sourceRule, KifList forallExpr, MatchResult matchResult) {
            ...
            var variables = KifTerm.collectVariablesFromSpec(forallExpr.get(1));
            if (variables.isEmpty()) {
                processDerivedStandard(sourceRule, body, matchResult);
                return;
            }
            ...
            if (ENABLE_RULE_DERIVATION && body.getOperator().filter(op -> op.equals(KIF_OPERATOR_IMPLIES) || op.equals(KIF_OPERATOR_EQUIV)).isPresent()) {
                ...
                var pa = new PotentialAssertion(forallExpr, calculateDerivedPri(matchResult.supportIds(), sourceRule.pri()), matchResult.supportIds(),
                        sourceRule.id(), false, false, false,
                        findCommonSourceNodeId(matchResult.supportIds()), AssertionType.UNIVERSAL, List.copyOf(variables), newDepth);
                submitPotentialAssertion(pa);
            } else {
                System.err.println("Warning: Rule derivation/Forward instantiation disabled, ignoring derived 'forall': " + forallExpr.toKifString());
            }
        }

        private void processDerivedExists(Rule sourceRule, KifList existsExpr, MatchResult matchResult) {
            if (!ENABLE_SKOLEMIZATION) {
                System.err.println("Warning: Skolemization disabled, ignoring derived 'exists': " + existsExpr.toKifString());
                return;
            }
            if (existsExpr.size() != 3 || !(existsExpr.get(1) instanceof KifList || existsExpr.get(1) instanceof KifVar) || !(existsExpr.get(2) instanceof KifList body)) {
            ...
            submitPotentialAssertion(new PotentialAssertion(skolemizedBody, calculateDerivedPri(matchResult.supportIds(), sourceRule.pri()), matchResult.supportIds(),
                    sourceRule.id(), isEq, isNegated, isOriented,
                    findCommonSourceNodeId(matchResult.supportIds()), AssertionType.SKOLEMIZED, List.of(), newDepth));
        }
        ...
        private void processDerivedStandard(Rule sourceRule, KifList derivedTerm, MatchResult matchResult) {
            ...
            submitPotentialAssertion(new PotentialAssertion(derivedTerm, calculateDerivedPri(matchResult.supportIds(), sourceRule.pri()), matchResult.supportIds(),
                    sourceRule.id(), isEq, isNegated, isOriented,
                    findCommonSourceNodeId(matchResult.supportIds()), derivedType, List.of(), newDepth));
        }
        ...
        private Stream<MatchResult> findMatchesRecursive(Rule rule, List<KifTerm> remainingClauses, Map<KifVar, KifTerm> currentBindings, Set<String> currentSupportIds) {
            ...
            return kb.findUnifiableAssertions(pattern)
                    ...
                    .flatMap(candidate -> ofNullable(Unifier.unify(pattern, candidate.getEffectiveTerm(), currentBindings))
                            .map(newBindings -> {
        ...
        private void triggerInstantiation(Assertion groundAssertion) {
            if (groundAssertion.derivationDepth() >= MAX_DERIVATION_DEPTH || !(groundAssertion.kif().get(0) instanceof KifAtom predicate))
                return;
                                    ...
                                    if (bindings.keySet().containsAll(quantifiedVars)) {
                                        ...
                                        if (instantiatedFormula instanceof KifList instantiatedList
                                            ...
                                            if (newDepth <= MAX_DERIVATION_DEPTH) {
                                                ...
                                                submitPotentialAssertion(new PotentialAssertion(instantiatedList, newPri, newSupport, uniAssertion.id(),
                                                        isEq, isNeg, isOriented, findCommonSourceNodeId(newSupport),
                                                        derivedType, List.of(), newDepth));
                                            }
                                        }
                                    }
        ...
        private Stream<Map<KifVar, KifTerm>> findSubExpressionMatches(KifTerm expression, KifTerm target) {
            return Stream.concat(
                    ofNullable(Unifier.match(expression, target, Map.of())).stream(),
                    (expression instanceof KifList list) ?
                            list.terms().stream().flatMap(sub -> findSubExpressionMatches(sub, target)) : Stream.empty()
            );
        }
        ...
        KifList performSkolemization(KifList body, Collection<KifVar> existentialVars, Map<KifVar, KifTerm> contextBindings) {
            ...
            var skolemArgs = freeVarsInBody.stream()
                    .map(fv -> Unifier.substFully(contextBindings.getOrDefault(fv, fv), contextBindings))
                    .sorted(Comparator.comparing(KifTerm::toKifString))
                    .toList();

            Map<KifVar, KifTerm> skolemMap = new HashMap<>();
            for (var exVar : existentialVars) {
                var skolemTerm = skolemArgs.isEmpty()
                        ? KifAtom.of(ID_PREFIX_SKOLEM_CONST + exVar.name().substring(1) + "_" + idCounter.incrementAndGet())
                        : new KifList(Stream.concat(
                        Stream.of(KifAtom.of(ID_PREFIX_SKOLEM_FUNC + exVar.name().substring(1) + "_" + idCounter.incrementAndGet())),
                        skolemArgs.stream()
                ).toList());
            ...
            rules.forEach(rule -> rule.antecedents().forEach(clause -> {
                ...
                if (clauseIsNegated == newAssertion.isNegated()) {
                    ...
                    ofNullable(Unifier.unify(pattern, newAssertion.getEffectiveTerm(), Map.of()))
                            .ifPresent(bindings -> queueTask(InferenceTask.matchAntecedent(rule, newAssertion, bindings, calculateTaskPri(rule.pri(), newAssertion.pri()))));
                }
            }));
        }
        ...
        private void triggerRewriteTasks(Assertion newAssertion) {
            ...
            if (newAssertion.isEquality() && newAssertion.isOrientedEquality() && !newAssertion.isNegated() && newAssertion.kif().size() == 3) {
                kb.findUnifiableAssertions(newAssertion.kif().get(1))
                        ...
                        .forEach(target -> queueTask(InferenceTask.applyRewrite(newAssertion, target, calculateTaskPri(newAssertion.pri(), target.pri()))));
            }

            kb.getAllAssertions().stream()
                    .filter(a -> a.assertionType() == AssertionType.GROUND || a.assertionType() == AssertionType.SKOLEMIZED)
                    .filter(a -> a.isEquality() && a.isOrientedEquality() && !a.isNegated() && a.kif().size() == 3)
                    .filter(ruleAssertion -> !ruleAssertion.id().equals(newAssertion.id()))
                    .filter(ruleAssertion -> Unifier.match(ruleAssertion.kif().get(1), newAssertion.getEffectiveTerm(), Map.of()) != null)
                    .forEach(ruleAssertion -> queueTask(InferenceTask.applyRewrite(ruleAssertion, newAssertion, calculateTaskPri(ruleAssertion.pri(), newAssertion.pri()))));
        }
            ...
            kb.getAllAssertions().stream()
                    ...
                    .forEach(existing -> newRule.antecedents().forEach(clause -> {
                        ...
                        if (clauseIsNegated == existing.isNegated()) {
                            var pattern = clauseIsNegated ? ((KifList) clause).get(1) : clause;
                            ofNullable(Unifier.unify(pattern, existing.getEffectiveTerm(), Map.of()))
                                    .ifPresent(bindings -> queueTask(InferenceTask.matchAntecedent(newRule, existing, bindings, calculateTaskPri(newRule.pri(), existing.pri()))));
                        }
                    }));
        }

        private void queueTask(InferenceTask task) {
            if (!taskQueue.offer(task)) {
                System.err.println("Warning: Task queue full. Discarding task: " + task.type());
            }
            checkQueueThresholds();
        }
        ...
        @Nullable
        String findCommonSourceNodeId(Set<String> supportIds) {
        ...
        private KifList simplifyLogicalTerm(KifList term) {
            ...
            if (!term.equals(current) && MAX_SIMPLIFY_DEPTH <= 5)
        ...
        private KifList simplifyLogicalTermOnce(KifList term) {
            if (term.getOperator().filter(KIF_OPERATOR_NOT::equals).isPresent() && term.size() == 2 && term.get(1) instanceof KifList negList) {
                if (negList.getOperator().filter(KIF_OPERATOR_NOT::equals).isPresent() && negList.size() == 2 && negList.get(1) instanceof KifList innerList) {
                    return simplifyLogicalTermOnce(innerList);
                }
            }
        ...
        private void checkQueueThresholds() {
            ...
            var commitQSize = commitQueue.size();
            var taskHalt = TASK_QUEUE_CAPACITY * 9 / 10;
            var commitHalt = COMMIT_QUEUE_CAPACITY * 9 / 10;

            if (taskQSize >= taskHalt || commitQSize >= commitHalt) {
    ...
    static class Unifier {
        ...
        private static Map<KifVar, KifTerm> matchRecursive(KifTerm pattern, KifTerm term, Map<KifVar, KifTerm> bindings, int depth) {
            ...
            if (patternSubst instanceof KifVar varP)
                return bindVariable(varP, term, bindings, false, depth);
        ...
        private static KifTerm substRecursive(KifTerm term, Map<KifVar, KifTerm> bindings, int depth) {
            ...
            return switch (term) {
                ...
                case KifVar var -> ofNullable(bindings.get(var))
                        .map(boundTerm -> substRecursive(boundTerm, bindings, depth + 1))
                        .orElse(var);
                case KifList list -> {
                    var newTerms = list.terms().stream()
        ...
        private static Map<KifVar, KifTerm> bindVariable(KifVar var, KifTerm value, Map<KifVar, KifTerm> bindings, boolean doOccursCheck, int depth) {
            ...
            if (bindings.containsKey(var)) {
                return doOccursCheck ? unifyRecursive(bindings.get(var), value, bindings, depth + 1)
                        : matchRecursive(bindings.get(var), value, bindings, depth + 1);
            }
        ...
        private static Optional<KifTerm> rewriteRecursive(KifTerm target, KifTerm lhsPattern, KifTerm rhsTemplate, int depth) {
            ...
            return ofNullable(matchRecursive(lhsPattern, target, Map.of(), depth + 1))
                    .map(bindings -> substRecursive(rhsTemplate, bindings, depth + 1))
                    .or(() -> (target instanceof KifList targetList)
    ...
    static class SwingUI extends JFrame {
        ...
        private final JMenuItem analyzeItem = new JMenuItem("Analyze Note (LLM -> KIF)"),
                enhanceItem = new JMenuItem("Enhance Note (LLM Replace)"),
                summarizeItem = new JMenuItem("Summarize Note (LLM)"),
                keyConceptsItem = new JMenuItem("Identify Key Concepts (LLM)"),
                generateQuestionsItem = new JMenuItem("Generate Questions (LLM)"),
                renameItem = new JMenuItem("Rename Note"),
                removeItem = new JMenuItem("Remove Note");
        ...
        private final JMenuItem retractAssertionItem = new JMenuItem("Retract Assertion"),
                showSupportItem = new JMenuItem("Show Support Chain"),
                viewRulesItem = new JMenuItem("View Rules");
        ...
        private void setupFonts() {
            Stream.of(noteList, noteEditor, addButton, pauseResumeButton, clearAllButton, statusLabel,
                            ...
                            retractAssertionItem, showSupportItem, viewRulesItem)
        ...
        private void setupLayout() {
            ...
            var mainSplitPane = mainPane(leftPane);
        ...
        private JSplitPane mainPane(JScrollPane leftPane) {
        ...
        private void setupWindowListener() {
            addWindowListener(new WindowAdapter() {
                ...
                public void windowClosing(WindowEvent e) {
                    saveCurrentNoteText();
                    ofNullable(SwingUI.this.reasoner).ifPresent(CogNote::stopReasoner);
                    dispose();
                    System.exit(0);
                }
            });
        ...
        private void updateUIForSelection() {
            ...
            ofNullable(reasoner).ifPresent(r -> pauseResumeButton.setText(r.isPaused() ? "Resume" : "Pause"));
        ...
        private void addNote(ActionEvent e) {
            ofNullable(JOptionPane.showInputDialog(this, "Enter note title:", "New Note", JOptionPane.PLAIN_MESSAGE))
                    ...
                    .ifPresent(title -> {
                        ...
                        noteAssertionModels.put(newNote.id, new DefaultListModel<>());
        ...
        private void removeNoteAction(ActionEvent e) {
            performNoteAction("Removing", "Confirm Removal", "Remove note '%s' and retract all associated assertions (including derived)?", JOptionPane.WARNING_MESSAGE, noteToRemove -> {
                ...
                noteAssertionModels.remove(noteToRemove.id);
        ...
        private void renameNoteAction(ActionEvent e) {
            ofNullable(currentNote)
                    .flatMap(note -> ofNullable(JOptionPane.showInputDialog(this, "Enter new title for '" + note.title + "':", "Rename Note", JOptionPane.PLAIN_MESSAGE, null, null, note.title))
                            .map(Object::toString).map(String::trim).filter(Predicate.not(String::isEmpty))
                            .filter(newTitle -> !newTitle.equals(note.title))
                            .map(newTitle -> {
                                note.title = newTitle;
                                return note;
                            })
        ...
        private void retractSelectedAssertionAction(ActionEvent e) {
            ofNullable(noteAssertionDisplayList.getSelectedValue())
        ...
        private void showSupportAction(ActionEvent e) {
            ofNullable(noteAssertionDisplayList.getSelectedValue())
                    .filter(AssertionViewModel::isActualAssertion)
                    .map(AssertionViewModel::id)
                    .flatMap(id -> reasoner.knowledgeBase.getAssertion(id))
                    .ifPresent(this::displaySupportChain);
        }

        private void viewRulesAction(ActionEvent e) {
            if (reasoner == null) return;
            var rulesText = reasoner.reasonerEngine.getRules().stream()
                    .sorted(Comparator.comparing(Rule::id))
                    .map(r -> String.format("[%s] %.2f %s", r.id(), r.pri(), r.form().toKifString()))
                    .collect(Collectors.joining("\n"));
        ...
        private void togglePause(ActionEvent e) {
            ofNullable(reasoner).ifPresent(r -> r.setPaused(!r.isPaused()));
        }

        private void clearAll(ActionEvent e) {
            ofNullable(reasoner).ifPresent(r -> {
        ...
        private void performNoteAction(String actionName, String confirmTitle, String confirmMsgFormat, int confirmMsgType, Consumer<Note> action) {
            ofNullable(currentNote).filter(_ -> reasoner != null).ifPresent(note -> {
                if (JOptionPane.showConfirmDialog(this, String.format(confirmMsgFormat, note.title), confirmTitle, JOptionPane.YES_NO_OPTION, confirmMsgType) == JOptionPane.YES_OPTION) {
                    ...
                    try {
                        ...
                        setControlsEnabled(true);
                        ofNullable(reasoner).ifPresent(CogNote::updateUIStatus);
                    }
                }
            });
        }
        ...
        private <T> void performNoteActionAsync(String actionName, NoteAsyncAction<T> asyncAction, BiConsumer<T, Note> successCallback, BiConsumer<Throwable, Note> failureCallback) {
            ofNullable(currentNote).filter(_ -> reasoner != null).ifPresent(noteForAction -> {
        ...
        private void processLlmEnhancementResponse(String enhancedText, Note enhancedNote) {
            ...
            if (enhancedText != null && !enhancedText.isBlank()) {
                ...
                noteEditor.setCaretPosition(0);
                ofNullable(reasoner).ifPresent(r -> r.reasonerStatus = String.format("Enhanced note '%s'", enhancedNote.title));
                addLlmUiItem(enhancedNote.id, AssertionDisplayType.LLM_INFO, "Note text enhanced by LLM.");
            } else {
                ofNullable(reasoner).ifPresent(r -> r.reasonerStatus = String.format("Enhancement failed for '%s': Empty response", enhancedNote.title));
        ...
        private void processLlmKifResponse(String kifString, Note analyzedNote) {
            removeLlmPlaceholders(analyzedNote.id);
            try {
                ...
                for (var term : terms) {
                    if (term instanceof KifList list && !list.terms().isEmpty()) {
                        ofNullable(reasoner).ifPresent(r -> r.queueExpressionFromSource(term, "UI-LLM:" + analyzedNote.id, analyzedNote.id));
                        counts[0]++;
                    } else if (term != null) {
                ...
                ofNullable(reasoner).ifPresent(r -> r.reasonerStatus = String.format("Analyzed '%s': %d queued, %d skipped", analyzedNote.title, counts[0], counts[1]));
        ...
        private void handleLlmFailure(Throwable ex, Note contextNote) {
            ...
            ofNullable(reasoner).ifPresent(r -> r.reasonerStatus = action + " for " + contextNote.title);
        ...
        public void handleReasonerCallback(String type, Object payload, @Nullable String associatedNoteId) {
            SwingUtilities.invokeLater(() -> {
                switch (payload) {
                    case Assertion assertion -> handleAssertionCallback(type, assertion, associatedNoteId);
                    case AssertionViewModel llmItem when type.equals(CALLBACK_LLM_RESPONSE) ->
                            handleLlmCallback(llmItem);
                    default -> {
                    }
                }
            });
        ...
        private void handleAssertionCallback(String type, Assertion assertion, @Nullable String associatedNoteId) {
            var vm = AssertionViewModel.fromAssertion(assertion, type, associatedNoteId);
            var status = vm.status();

            if (associatedNoteId != null) {
                ofNullable(noteAssertionModels.get(associatedNoteId))
                        .ifPresent(model -> updateOrAddModelItem(model, vm));
            }

            if (status == AssertionStatus.RETRACTED || status == AssertionStatus.EVICTED) {
                updateAssertionStatusInOtherNoteModels(assertion.id(), status, associatedNoteId);
                highlightAffectedNoteText(assertion, status);
            } else if (status == AssertionStatus.ACTIVE) {
                highlightAffectedNoteText(assertion, status);
            }
        }

        private void handleLlmCallback(AssertionViewModel llmItem) {
            ofNullable(llmItem.noteId())
                    .map(noteAssertionModels::get)
                    .ifPresent(model -> updateOrAddModelItem(model, llmItem));
        }
        ...
        private void updateOrAddModelItem(DefaultListModel<AssertionViewModel> model, AssertionViewModel newItem) {
            ...
            if (existingIndex != -1) {
                ...
                if (newItem.status() != existingItem.status() || !newItem.content().equals(existingItem.content()) || !Objects.equals(newItem.associatedNoteId(), existingItem.associatedNoteId())) {
        ...
        private void addLlmUiItem(String noteId, AssertionDisplayType type, String content) {
            var vm = new AssertionViewModel(
                    ...
                    noteId, content, type, AssertionStatus.ACTIVE,
                    0.0, -1, System.currentTimeMillis(), noteId
            );
            ofNullable(noteAssertionModels.get(noteId)).ifPresent(model -> updateOrAddModelItem(model, vm));
            ofNullable(reasoner).ifPresent(r -> r.callback(CALLBACK_LLM_RESPONSE, vm));
        }
        ...
        private void displaySupportChain(Assertion startingAssertion) {
            ...
            while (!queue.isEmpty()) {
                ...
                reasoner.knowledgeBase.getAssertion(currentId).ifPresent(assertion -> {
                    var displayNoteId = reasoner.reasonerEngine.findCommonSourceNodeId(assertion.support());
                    if (displayNoteId == null && assertion.sourceNoteId() != null)
                        displayNoteId = assertion.sourceNoteId();
                    var vm = AssertionViewModel.fromAssertion(assertion, CALLBACK_ASSERT_ADDED, displayNoteId);
                    model.addElement(vm);
                    assertion.support().forEach(queue::offer);
                });
            }
            var sortedList = Collections.list(model.elements());
        ...
        private void highlightAffectedNoteText(Assertion assertion, AssertionStatus status) {
            if (reasoner == null || currentNote == null) return;

            var commonNoteId = reasoner.reasonerEngine.findCommonSourceNodeId(assertion.support());
            if (commonNoteId == null && assertion.derivationDepth() == 0) {
                commonNoteId = assertion.sourceNoteId();
            }

            if (currentNote.id.equals(commonNoteId)) {
        ...
        private String extractHighlightTerm(KifList kif) {
            return kif.getOperator()
                    .filter(op -> !op.matches("and|or|not|=>|<=>|=|forall|exists"))
                    .or(() -> (kif.size() > 1 && kif.get(1) instanceof KifAtom(
                            var value
                    )) ? Optional.of(value) : Optional.empty())
                    .or(() -> (kif.size() > 2 && kif.get(2) instanceof KifAtom(
                            var value
                    )) ? Optional.of(value) : Optional.empty())
                    .filter(s -> s.length() > 2)
                    .orElse(null);
        ...
        private void setControlsEnabled(boolean enabled) {
            var noteSelected = (currentNote != null);
            var reasonerReady = (reasoner != null && reasoner.running && !reasoner.paused);

            Stream.of(addButton, clearAllButton).forEach(c -> c.setEnabled(enabled && reasoner != null));
            pauseResumeButton.setEnabled(enabled && reasoner != null && reasoner.running);

            Stream.of(analyzeItem, enhanceItem, summarizeItem, keyConceptsItem, generateQuestionsItem, removeItem)
                    .forEach(c -> c.setEnabled(enabled && noteSelected && reasonerReady));
            renameItem.setEnabled(enabled && noteSelected);

            viewRulesItem.setEnabled(enabled && reasoner != null);

            if (reasoner != null && reasoner.running) {
        ...
        record AssertionViewModel(String id, @Nullable String noteId, String content, AssertionDisplayType displayType,
            ...
            static AssertionViewModel fromAssertion(Assertion assertion, String callbackType, @Nullable String associatedNoteId) {
                var type = switch (assertion.assertionType()) {
                    case GROUND ->
                            (assertion.derivationDepth() == 0) ? (callbackType.equals(CALLBACK_ASSERT_INPUT) ? AssertionDisplayType.INPUT : AssertionDisplayType.ADDED) : AssertionDisplayType.DERIVED;
                ...
                return new AssertionViewModel(assertion.id(), assertion.sourceNoteId(), assertion.toKifString(), type, stat,
                        assertion.pri(), assertion.derivationDepth(), assertion.timestamp(), associatedNoteId);
            }
        ...
        static class AssertionListCellRenderer extends JPanel implements ListCellRenderer<AssertionViewModel> {
            ...
            public Component getListCellRendererComponent(JList<? extends AssertionViewModel> list, AssertionViewModel value, int index, boolean isSelected, boolean cellHasFocus) {
                ...
                if (value.isActualAssertion()) {
                    details = String.format("P:%.3f | D:%d | %s%s", value.priority(), value.depth(), timeFormatter.format(Instant.ofEpochMilli(value.timestamp())),
                            value.associatedNoteId() != null ? " (N:" + value.associatedNoteId().replace(ID_PREFIX_NOTE, "") + ")" : "");
                } else {
